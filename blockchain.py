...
class Client:
    ...
    def __push_data(self, amount, recepient):
        self.txdatawithoutsign = str(amount) + " " + publickeylist[self.name].export_key(format='PEM') + " " + \
                                 publickeylist[recepient.name].export_key(format='PEM')
        mempool.append(
            [amount, publickeylist[self.name], publickeylist[recepient.name], self.__sign_data(self.txdatawithoutsign)])
        if len(mempool) == 5:
